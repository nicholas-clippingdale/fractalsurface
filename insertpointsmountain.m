function vertnew = insertpointsmountain(vert, d)
% Generates a more detailed vertices matrix for a mountain.
%
% insertpointsmountain() takes a matrix of vertices with values for all
% three axes. Randomly determined points, perturbed along all three axes,
% are then placed in the gaps to produce a more detailed surface. This
% particular function will create the vertices matrix for a mountain. Only
...
diag = (diag1 + diag2)./2;
da = d*diag.*rand(n-1,n-1);
newz(2:2:2*n-1,2:2:2*n-1) = 0.25 * (z(1:n-1,1:n-1) + z(2:n,1:n-1) ...
    + z(1:n-1,2:n) + z(2:n,2:n)) + da;
%%%%%%%%%%%%%%%% Scale the Matrix to Produce the Mountain %%%%%%%%%%%%%%%%
mu = [0.5, 0.5];
Sigma = [0.16 0; 0 0.16];
x1 = linspace(0,1,m);
x2 = linspace(0,1,m);
[X1,X2] = meshgrid(x1,x2);
scale = mvnpdf([X1(:) X2(:)], mu, Sigma);
scale = reshape(scale, length(x2), length(x1));
newz = newz .* scale;
